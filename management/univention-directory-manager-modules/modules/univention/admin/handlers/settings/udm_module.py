...
property_descriptions = {
	...
	'umcmessagecatalog': univention.admin.property(
		...
		long_description='GNU message catalog (syntax: <language tag> <Base64 encoded GNU message catalog>)',
		syntax=univention.admin.syntax.Localesubdirname_and_GNUMessageCatalog_umc,
		multivalue=True,
	),
...
messagecatalog_mappings = {
	...
	'umcmessagecatalog': 'univentionUMCMessageCatalog;entry-lang-'
}
			...
			for property_name, old_value, new_value in diff:
				if property_name == udm_attr:
					...
					for language_tag, old_mo_data_base64 in old_dict.items():
						ldap_attribute = ''.join((ldap_attr, language_tag))
